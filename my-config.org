...
** Some notes about emacs, vim and everything in between (evil-mode)
   ...
   /editors—spirals into despair before he succumbs to the dark side./
   /This is his config./
...
*** Rebetika collection
    ...
    artist, coposer etc) are stored in properties.
    ...
    (use-package emacs
      :after org counsel
      :config
      (defun l/rebet-id3-tags-to-org-props ()
	"For my rebetika lyrics archive, which is an org file. Reads id3 tags
    from a selected mp3 file and converts them to org properties. Also, if the
    title of the track is different from the org header, it replaces the header
    with the title. Depends on ivy and counsel-fzf."
	(interactive)
	(let* ((song-data
		(mapcar
		 ...
		 (seq-remove
		  ...
		  (process-lines
		   ...
		   (ivy-read "Get props from:"
			     ...
			     :action (lambda (x) (eval x))
			     :caller 'l/rebet-id3-tags-to-org-props))))))
	  (outline-back-to-heading)
	  (org-beginning-of-line)
	  (let ((beg (point))
		(title (nth 0 (cdr (assoc "TAG:title" song-data)))))
	    (org-end-of-line)
	    (if (not (string= title (buffer-substring-no-properties beg (point))))
		(progn
		  (delete-region beg (point))
		  (insert title))))
	  ;; set the properties. If no id3 tag is present, prompts for manual insertion
      ...
      :bind
      (:map leader-org
	    ("rt" . l/rebet-id3-tags-to-org-props)))
    #+end_src
