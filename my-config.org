...
**** Get subtitles:
     ...
     Use [[https://github.com/Diaoul/subliminal][subliminal]] to find subtitles online. The gist of using 'start-process' comes from [[http://ergoemacs.org/emacs/elisp_call_shell_command.html][here]].

     #+begin_src emacs-lisp
     (defun get-subtitles ()
       "Get subtitles for marked files while in a dired buffer. Depends on subliminal, so it has to be installed on your system."
       (interactive)
	 ...
	 (let* ((lang-list '("en" "gr"))
	   ...
	   (mapc
		...
		(start-process "get-subtitles" "*get-subtitles*" "subliminal" "download" "-l" lang x)))
