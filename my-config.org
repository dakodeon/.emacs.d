...
#+begin_src emacs-lisp
  (defun compose-attach-marked-files ()
    "Compose mail and attach all the marked files from a dired buffer. Uses `compose-mail', or `org-msg-attach-attach' if org-msg is enabled."
    (interactive)
...
*** Dired overall
    ...
    (use-package dired
      ...
      :config
      (remove-hook 'dired-mode-hook 'ranger-set-dired-key) ;; this is a bug??
	...
	'(progn
	   ...
	   (evil-define-key 'normal dired-mode-map (kbd "z") 'dired-omit-mode)
	   (evil-define-key 'normal dired-mode-map (kbd "g") 'louk/dired-nav-map)
...
*** Peep dired
    ...
    (use-package peep-dired
      ...
      :config
      ;; custom functions for better peep navigation
      (defun louk/peep-dired-up-directory ()
	"Immediately peep first file when moving up directory in peep-mode"
	(interactive)
	(dired-up-directory)
	(peep-dired-display-file-other-window))

      (defun louk/peep-dired-find-file ()
	"Immediately peep first file when entering a subdirectory. If the selection is a file, just go to the peeped window."
	(interactive)
	...
	"Immediately peep after visiting previous dirline."
	(interactive)
	(dired-prev-dirline 1)
	(peep-dired-display-file-other-window))

      (defun louk/peep-dired-next-dirline ()
	"Immediately peep after visiting next dirline."
	(interactive)
...
(use-package dired-narrow
  :ensure t
  :after dired evil evil-collection
  :config
...
(use-package dired-ranger
  ...
  (eval-after-load "evil"
    '(progn
       (evil-define-key 'normal dired-mode-map (kbd "r c") 'dired-ranger-copy)
       (evil-define-key 'normal dired-mode-map (kbd "r m") 'dired-ranger-move)
       (evil-define-key 'normal dired-mode-map (kbd "r p") 'dired-ranger-paste))))
#+end_src
